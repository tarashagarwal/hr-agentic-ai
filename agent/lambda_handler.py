...
def lambda_handler(event, context):
    ...
    Returns: {"statusCode":200, "body": "<agent reply>"}
    """
    usr = event.get("input") or event.get("body", {}).get("input", "")
    state = build_initial_state(usr)
    result = graph.invoke(state)
    return {
        "statusCode": 200,
        "body": result["agent_outcome"].return_values["output"],
    }
